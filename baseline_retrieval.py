"""Generate Programs with Primitives using Retrieved Examples."""
import os
import torch
import argparse
import transformers
...
import random
from chroma_store import MathExampleStore
...
def main():
    ...
    print("Initializing example store...")
    example_store = MathExampleStore()
    
    # Load data
    print("Loading dataset...")
    examples = load_dataset(args.task_name, args.max_num_examples)
    template_path = os.path.join("prompt", args.task_name, "primitive_retrieval.md")
    ...
    class TempDataset(Dataset):
        def __init__(self, examples: list[dict]):
            ...
            for ex in self.examples:
                # We'll now just create the basic prompt without similar examples
                prompt_args = PROMPT_ARGS_FUNC[args.task_name](ex)
                prompt_args.update({
                    "toolbox": library_preview,
                    "similar_examples": []  # Empty list as we'll add examples later
                })
    ...
    for i in range(len(dataset)):
        ...
        # Get similar examples from previously solved examples
        current_question = examples[i]["question"]
        similar_examples = example_store.get_similar_examples(
            current_question, 
            k=args.num_examples_to_retrieve  # Can't retrieve more than we've solved
        )
        
        # Create the prompt with retrieved examples
        prompt_args = PROMPT_ARGS_FUNC[args.task_name](examples[i])
        prompt_args.update({
            "toolbox": library_preview,
            "similar_examples": similar_examples
        })
        current_prompt = template.render(**prompt_args)
        
        print(tokenizer(current_prompt)["input_ids"])

        # Generate response for current example
        outputs = pipeline(
            current_prompt,
        ...
        write_prompt(fw_log, current_prompt, library_preview, i)

        response_list = []
        for r in outputs:
            resp = extract_llama_response(
                output_text=r["generated_text"],
        ...
        best_response = response_list[best_index]
        
        # Add successful solutions to ChromaDB
        if best_response["is_success"]:
            example_store.add_example(
                question=current_question,
                solution=best_response["solution"]
            )

        result_list.append(best_response)
...
if __name__ == "__main__":
    ...
    # retrieval settings
    parser.add_argument("--num_examples_to_retrieve", type=int, default=2,
